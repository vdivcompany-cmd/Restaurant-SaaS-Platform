...
export class BranchRepository {
  public async create(tenantId: string, data: CreateBranchDto): Promise<IBranch> {
    return (await tenantQuery.create(BranchModel, tenantId, data)) as IBranch;
  }

  public async findAll(tenantId: string): Promise<IBranch[]> {
    return await tenantQuery.find(BranchModel, tenantId).sort({ isDefault: -1, createdAt: 1 }).exec();
  }
