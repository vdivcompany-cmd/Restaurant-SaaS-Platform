          ...
          properties: {
            ...
            variants: {
              ...
              items: {
                type: 'object',
                properties: {
                  name: { type: 'string', example: 'Drink Choice' },
                  minSelect: { type: 'integer', example: 0 },
                  ...
                  options: {
                    ...
                    items: {
                      type: 'object',
                      properties: {
                        name: { type: 'string', example: 'Diet Cola' },
                        priceDelta: { type: 'number', example: 25 },
                      },
                    },
                  },
                },
...
// When executed directly, generate docs/docs.json and backend/src/docs/docs.json
const spec = buildOpenApiSpec();
const jsonContent = JSON.stringify(spec, null, 2);

const rootDocsPath = path.resolve(__dirname, '../../docs/docs.json');
const backendDocsDir = path.resolve(__dirname, '../src/docs');
const backendDocsPath = path.resolve(backendDocsDir, 'docs.json');

fs.mkdirSync(path.dirname(rootDocsPath), { recursive: true });
fs.writeFileSync(rootDocsPath, jsonContent, 'utf-8');
console.log(`[Success] Written root OpenAPI spec to: ${rootDocsPath}`);

fs.mkdirSync(backendDocsDir, { recursive: true });
fs.writeFileSync(backendDocsPath, jsonContent, 'utf-8');
console.log(`[Success] Written backend OpenAPI spec to: ${backendDocsPath}`);
